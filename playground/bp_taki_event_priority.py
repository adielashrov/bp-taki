...
def player_behavior(index, num_of_cards=2):
    ...
    sync_statements = [
        {
          'waitFor' :general_player_event_set
        },
        {
            'request': cards_events,
            'requestPriority': 30
        },
        {
            'request': [ bp.BEvent(f"p_{index}_draw_card") ],
            'requestPriority': 30
        }
    ]

    while cards_events:
        lastEvent = yield bp.sync(multiSync=sync_statements)
        # print(f"player {index} received event :{lastEvent.name}")
        if lastEvent.name.startswith(f"p_{index}") and is_color_card_event(lastEvent):
            # print(f"Going to remove :{lastEvent.name} from cards collection {cards_events}")
            cards_events.remove(lastEvent)
        if lastEvent.name.startswith(f"p_{index}_draw_card"):
            continue
...
def enforce_turns():  # blocks moves that are not in turn
    yield bp.sync(waitFor=bp.BEvent("start_game"))
    while True:
        yield bp.sync(waitFor=any_player_0, hardBlock=any_player_1)
        yield bp.sync(waitFor=any_player_1, hardBlock=any_player_0)
