...
from bppy.model.event_selection.statement_priority_event_selection_strategy import \
    StatementPriorityBasedEventSelectionStrategy
...
def create_taki_color_block(color: str) -> bp.EventSet:
    ...
    def to_block(e: BPEvent) -> bool:
        ...
        if is_change_color_event(e):
            # TODO: Adiel: should we allow change_color during TAKI?
            return True
    ...
    if card_str_index != -1:
        ...
        if stop_str_index != -1:
            ...
            if plus_2_str_index != -1:
                ...
                change_color_index = event.name.find("selected_") # to handle the selected changed color
                if change_color_index != -1:
                    # Extract color from patterns like "change_color_red" or "p_0_change_color_red"
                    color = [c for c in COLORS if c in event.name][0]
                    if color in COLORS:
                        return color, "CHANGE_COLOR"
                    else:
                        logger.debug(
                            "Received change_color event with no color specified, defaulting to empty string.")
                        return "", "CHANGE_COLOR"
                else:
                    ...
                    if super_taki_index != -1:
                        ...
                        if taki_str_index != -1:
                            ...
                            if color in COLORS:
                                logger.debug(
                                    f"[DEBUG extract_card_color_and_type] Regular TAKI: {event.name} → Color: {color}, Type: TAKI")
                                return color, "TAKI"
...
def enforce_turns(num_of_players=2):
    next_or_stop_or_taki_lst = [BPEvent("next_turn", priority=10.0), bp.EventSet(is_stop_card_event),
                                bp.EventSet(is_any_taki_event)]
    ...
    while True:  # We should block the other player from playing out of turn in all the while loop.
        ...
        if is_any_taki_event(last_event):  # Super TAKI or TAKI played
            ...
            yield bp.sync(request=BPEvent("done_post_action", priority=17.5),
                          # priority here is higher than draw_card, but lower than closed_taki
                          block=all_other_player_cards_besides_special_cards(current_player))
        ...
        if is_regular_card_event(last_event):
            ...
            while True:
                ...
                if taki_event.name == "done_post_action":
                    ...
                    logger.debug(
                        f"[ENFORCE_RULES] Last card color: {last_taki_card_color}, type: {last_taki_card_type}")
                    logger.debug(f"{'=' * 60}")
                    break

                # Update tracking with each card played during TAKI
                if is_regular_card_event(taki_event) or is_change_color_event(taki_event) or is_stop_card_event(
                        taki_event):
                    ...
                    logger.info(
                        f"[ENFORCE_RULES] The 'last_card' state was NOT updated. Next player might face wrong rules.\n")
            ...
            different_colors_or_types_event_set = bp.EventSetsDifference(all_player_events(),
                                                                         init_selected_color_or_type_event_set(
                                                                             card_color, card_type))
            logger.debug(
                f"[ENFORCE_RULES] Post-TAKI placement rules: must match color={card_color} OR type={card_type}")
