...
class SimulationResultListener(LogBProgramRunnerListener):
    """
    Listener that records which player triggered the game-ending
    `p_<i>_no_more_cards` event so simulations can tally wins.
    """
    ...
    def event_selected(self, b_program, event):
        ...
        if self.winner is None and is_no_more_cards_event(event):
            self.winner = extract_player_id(event)
...
def init_b_program(listener: Optional[LogBProgramRunnerListener] = None):
    active_listener = listener or LogBProgramRunnerListener(logger=logger)
...
def run_single_game(seed: int, listener: Optional[LogBProgramRunnerListener] = None,
                    configure_logging: bool = True) -> Optional[int]:
    """
    Run a single Taki game with a specific random seed.

    Returns the winning player index when available.
    """
    if configure_logging:
        setup_logger()

    random.seed(seed)
    logger.info(f"Starting Taki Game BProgram Execution | seed={seed}")

    active_listener = listener or LogBProgramRunnerListener(logger=logger)
    b_program = init_b_program(listener=active_listener)
    b_program.run()

    return getattr(active_listener, "winner", None)


def run_simulation(num_games: int = 10, starting_seed: int = 0) -> dict[int, int]:
    """
    Run multiple games back-to-back, each with a different seed.

    Seeds advance deterministically starting from `starting_seed`.
    Returns a mapping of player index to wins.
    """
    setup_logger()
    ...
    for offset in range(num_games):
        ...
        listener = SimulationResultListener(logger=logger)
        winner = run_single_game(seed, listener=listener, configure_logging=False)

        if winner in results:
            results[winner] += 1
            logger.info(f"[SIMULATION] Game {offset + 1}/{num_games} | seed={seed} | winner=p_{winner}")
        else:
            logger.warning(f"[SIMULATION] Game {offset + 1}/{num_games} | seed={seed} | winner undetermined")

    logger.info(f"[SIMULATION] Completed {num_games} games | P0 wins: {results[0]} | P1 wins: {results[1]}")
    print(f"Simulation results - games: {num_games}, player0 wins: {results[0]}, player1 wins: {results[1]}")

    return results


def regular_execution_of_bp_program(seed: int = SEED):
    setup_logger()
    random.seed(seed)
    logger.info(f"Starting Taki Game BProgram Execution | seed={seed}")
...
if __name__ == "__main__":
    # Run a small simulation by default; adjust num_games/starting_seed as needed.
    run_simulation(num_games=100, starting_seed=0)
