...
| `plus_2_{color}` | Action card: forces the next player to draw 2 cards (or play another `plus_2` to stack) |
...
p_{index}_plus_2_{color}       # e.g. p_0_plus_2_green
...
is_action_card_event(event)    # True for stop, plus_2, change_color, taki, super_taki
...
*Example values for this placeholder:*
- *"Prefer playing action cards (stop, plus_2) over regular cards to pressure the opponent, especially when the opponent has few cards in hand."*
- *"Play cards that consolidate the hand color: prefer cards whose color matches the most frequent color in your current hand, to set up future TAKI sequences."*
- *"Hold change_color cards as long as the hand is large; use them aggressively when the hand has 3 or fewer cards."*

### What to implement

Write a strategy b-thread named `{STRATEGY_FUNCTION_NAME}` that:

1. **Follows the exact same turn lifecycle** as the reference implementations (deal phase → `start_game` → turn loop → `no_more_cards` termination).
2. **Only influences card selection through priority values**. Do not add `block=` arguments or request lifecycle events.
3. **Implements the strategy described above** by assigning priority values to cards when they enter the hand and/or dynamically updating priorities during the turn loop.
4. **Handles all card types correctly**: regular cards, action cards (stop, plus_2, change_color), TAKI sequences (including `closed_taki`), draw events, and the `done_post_action` protocol.

### Output format

Provide:
1. A helper function `add_card_with_{STRATEGY_FUNCTION_NAME}_priority(index, card_name, original_priority, card_events)` that assigns priorities when cards are added to the hand.
2. The `@bp.thread` decorated strategy b-thread function.
3. A brief docstring explaining the strategic rationale and the priority values chosen.

Do not include imports or any code outside these two functions — they will be integrated into an existing file.

---

## PART 6 — Common Pitfalls to Avoid

- **Missing `done_post_action` wait**: After playing any non-TAKI action card (stop, plus_2, change_color), you MUST `yield bp.sync(waitFor=BPEvent("done_post_action", ...))` before continuing. Skipping this causes a deadlock.
